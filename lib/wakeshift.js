...
var networkAPI = require('./api/network');
var setupAPI = require('./api/setup');
...
var WakeShift = function(){
	...
	t.apiHandler = function(req,res){
		switch(req.params.module){
			case 'network':{
				networkAPI.apiHandler(req,res);
				return;
			}break;
			case 'setup':{
				setupAPI.apiHandler(req,res);
				return;
			}break;
		}
	...
	var receiveMessage = function(service, message, from){
		switch(message.action){
			case 'confirm_connection':{
				console.log('web client ready to receive messages');
			}break;
			default:{
	...
	t.socketHandler = function(sock){
		...
		sock.on('_',receiveMessage.bind(this,service));
